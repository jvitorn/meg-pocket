...
export default function CampanhasPage() {
  ...
  const [campanhaSelecionada, setCampanhaSelecionada] =
    useState<CampanhaInterface | null>(null);
  ...
  useEffect(() => {
    const fetchCampanhas = async () => {
      try {
        const res = await fetch("/api/campanhas/all");
        if (!res.ok) throw new Error("Erro ao carregar campanhas");
        const data: CampanhaInterface[] = await res.json();
        setCampanhas(data);
      } catch (err) {
  ...
  // Abre detalhes e carrega personagens
  ...
  return (
    <>
              ...
              >
                ...
                <div className="relative w-full h-44 sm:h-52 overflow-hidden">
                  ...
                  <div className="absolute left-4 right-4 bottom-4 rounded-md bg-black/40 backdrop-blur-sm px-4 py-3 border border-white/5">
                    ...
                    <p className="text-xs text-white/70 mt-1">
                      {campanha.count_jogadores} jogador
                      {campanha.count_jogadores !== 1 ? "es" : ""}
                    </p>
                  </div>
                </div>
                ...
                <div className="p-4 flex flex-col justify-between h-[170px] bg-linear-to-b from-transparent to-black/5">
                  ...
                  {campanha.tags && campanha.tags.length > 0 && (
                    <div className="flex flex-wrap gap-2 mb-4">
                      {campanha.tags.map((tag) => (
                        <span
                          key={tag}
                          className="text-xs px-2 py-1 rounded-full bg-white/5 text-white/80 border border-white/6 capitalize"
                        >
                          {tag}
                        </span>
                      ))}
                  ...
                  >
                    ...
                    <button
                      onClick={() => abrirDetalhes(campanha)}
                      className="flex-1 inline-flex items-center justify-center rounded-lg px-3 py-2 text-sm font-medium border border-white/10 hover:bg-white/5 transition"
                    >
                      Detalhes
                    </button>
                  </div>
      ...
      {/* Modal com animação suave — CORREÇÃO: não usar `asChild` para evitar React.Children.only */}
      <AnimatePresence>
        {dialogAberto && campanhaSelecionada && (
          <Dialog open={dialogAberto} onOpenChange={setDialogAberto}>
            <DialogContent
              className="max-w-4xl p-0 border-none bg-background/95 backdrop-blur-md sm:rounded-lg overflow-hidden"
            >
              {/* título acessível */}
              ...
              >
                ...
                <div className="relative w-full shrink-0 h-60 md:h-72 overflow-hidden">
                  ...
                  <div className="absolute bottom-4 left-6 text-white drop-shadow-lg">
                    ...
                    <p className="text-sm opacity-90">
                      Mestre: {campanhaSelecionada.mestre || "Desconhecido"}
                    </p>
                  </div>
                </div>
                  ...
                  {campanhaSelecionada.tags?.length ? (
                    <div className="flex flex-wrap gap-2 mt-3">
                      {campanhaSelecionada.tags.map((tag) => (
                        <span
                          key={tag}
                          className="text-xs px-2 py-1 rounded-full bg-muted/20 capitalize border border-border/20"
                        >
                          {tag}
                        </span>
                      ))}
                  ...
                  <section className="mt-6">
                    <div className="flex items-center justify-between mb-3">
                      ...
                      <span className="text-sm text-muted-foreground">
                        {loadingPersonagens
                          ? "Carregando..."
                          : `${personagens.length} personagem${
                              personagens.length !== 1 ? "s" : ""
                            }`}
                      </span>
                    </div>
                      ...
                      >
                        {personagens.map((p) => (
                          <motion.div
                            ...
                            className="flex items-center gap-3 rounded-md border p-3 bg-background/70 hover:bg-accent/10 transition"
                          >
                            {((p as any).imagem_pixel
                              ? (p as any).imagem_pixel
                              : p.url_imagem) ? (
                              <img
                                src={
                ...
                <div className="sticky bottom-0 border-t bg-background/95 p-4 flex flex-col sm:flex-row justify-between items-center gap-3">
                  <div className="text-sm text-muted-foreground">
                    {campanhaSelecionada.count_jogadores ?? 0} jogadores
                    registrados
                  </div>
                  <div className="flex items-center gap-3">
                    ...
                    <button
                      ...
                      className="inline-flex items-center justify-center rounded-md px-3 py-2 text-sm font-medium border hover:bg-white/5 transition"
                    >
                      Fechar
                    </button>
                  </div>
