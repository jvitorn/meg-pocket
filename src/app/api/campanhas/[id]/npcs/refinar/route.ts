...
import { NpcCampanhaError, refinarNarrativaNpc } from "@/lib/regras/campanhaNpc";
...
export async function POST(
  ...
  try {
    ...
    const descricao = await refinarNarrativaNpc(body?.npc ?? body, body?.estilo);

    return NextResponse.json({ ok: true, descricao });
  } catch (error) {
    ...
    console.error("Erro ao refinar narrativa do NPC:", error);
    return NextResponse.json(
      { ok: false, error: "Erro interno ao refinar narrativa." },
      { status: 500 }
    );
  }
}
