    ...
    <div className="space-y-5">
      <div className="space-y-2">
        <div className="flex flex-wrap items-center gap-2">
          <span className="rounded-full border border-amber-500/30 bg-amber-500/10 px-2 py-1 text-[11px] font-medium text-amber-100">
            {ITEM_TIPO_LABEL[item.tipo]}
          </span>
          <span className="rounded-full border border-emerald-500/30 bg-emerald-500/10 px-2 py-1 text-[11px] font-medium text-emerald-100">
            x{item.quantidade}
          </span>
          {effectLabel ? (
            <span className="inline-flex items-center gap-1 rounded-full border border-sky-500/30 bg-sky-500/10 px-2 py-1 text-[11px] font-medium text-sky-100">
      ...
      {item.esgotado ? (
        <div className="rounded-xl border border-dashed border-amber-500/25 bg-amber-500/[0.04] px-3 py-3 text-sm text-amber-100/75">
          Este item foi esgotado e permanece oculto apenas como registro.
        </div>
      ) : null}

      {defenseActive ? (
        <div className="rounded-xl border border-sky-500/25 bg-sky-500/[0.06] px-3 py-3 text-sm text-sky-100/80">
      ...
      {defenseBlocked ? (
        <div className="rounded-xl border border-sky-500/25 bg-sky-500/[0.06] px-3 py-3 text-sm text-sky-100/80">
          Já existe outra defesa ativa na ficha. Zere a barra de defesa antes de reutilizar este item.
        </div>
      ) : null}
        ...
        <DrawerContent>
          <div className="mx-auto w-full max-w-2xl">
            <DrawerHeader>
              ...
              <DrawerDescription>
                Use o item a partir da ficha. Cadastro e ajustes estruturais ficam com a administração.
              </DrawerDescription>
            </DrawerHeader>
      ...
      >
        <div className="space-y-4">
          <div className="rounded-2xl border border-amber-500/20 bg-amber-500/[0.07] p-4">
            <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
              <div>
                ...
                <p className="mt-1 text-lg font-semibold text-foreground">
                  {formatSlots(resumo.slotsOcupados)} / {formatSlots(resumo.slotsMaximos)} slots
                </p>
              ...
              <div className="flex items-center gap-3 rounded-xl border border-amber-500/20 bg-background/50 px-4 py-3">
            ...
            <div className="mt-4 h-2 overflow-hidden rounded-full bg-amber-950/50">
              <div
          ...
          {itensAtivos.length ? (
              ...
              {itensAtivos.map((item) => {
                ...
                return (
                  <div
                    ...
                    role="button"
                    tabIndex={0}
                    className="rounded-2xl border border-amber-500/15 bg-card/70 p-4 text-left transition hover:border-amber-400/30 hover:bg-card/90"
                  >
                    <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                      <div className="space-y-2">
                        <div className="flex flex-wrap items-center gap-2">
                          ...
                          {effectLabel ? (
                            <span className="rounded-full border border-sky-500/30 bg-sky-500/10 px-2 py-1 text-[11px] font-medium text-sky-100">
                              {effectLabel}
                            </span>
                          ) : null}
                        </div>
                        ...
                        <div className="flex flex-wrap gap-2 text-xs text-amber-100/80">
                          <span className="rounded-full bg-amber-500/10 px-2.5 py-1">
                          ...
                          typeof item.durabilidadeMax === "number" ? (
                            <span className="rounded-full bg-amber-500/10 px-2.5 py-1">
                              Durabilidade {item.durabilidadeAtual}/{item.durabilidadeMax}
                            </span>
                          ) : null}
                        </div>
            ...
            <div className="rounded-2xl border border-dashed border-amber-500/20 bg-amber-500/[0.04] p-5 text-sm text-amber-100/80">
              Nenhum item ativo registrado para este personagem.
            </div>
          )}

          {itensOcultos.length ? (
            <div className="rounded-2xl border border-amber-500/15 bg-amber-500/[0.04]">
              <button
                type="button"
              ...
              {showHiddenItems ? (
                <div className="grid gap-3 border-t border-amber-500/10 px-4 py-4">
                  {itensOcultos.map((item) => (
                    <div
                      ...
                      role="button"
                      tabIndex={0}
                      className="rounded-xl border border-dashed border-amber-500/20 bg-background/30 px-4 py-3 text-left opacity-75 transition hover:opacity-100"
                    >
          ...
          <div className="flex items-center gap-2 rounded-xl border border-amber-500/15 bg-amber-500/[0.04] p-3 text-xs text-amber-100/75">
            <ShieldAlert className="h-4 w-4" />
            Cadastro e edição estrutural de itens ficam restritos ao administrador do sistema.
          </div>
        </div>
      </FichaSection>
